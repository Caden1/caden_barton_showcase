defmodule CadenBartonShowcaseWeb.BuildShowLive do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash}>
      <%= if @project do %>
        <section class="space-y-8 text-zinc-50">
          <div class="space-y-3">
            ...
            <h1 class="text-4xl font-bold leading-tight">{@project.name}</h1>
            <p class="text-zinc-200">{@project.summary}</p>
          </div>

          <div class="grid gap-6 lg:grid-cols-3">
            <article class="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-6 shadow-xl shadow-black/30 transition hover:-translate-y-1 hover:border-emerald-400/60">
      ...
      <% else %>
        <section class="space-y-4 text-zinc-50">
