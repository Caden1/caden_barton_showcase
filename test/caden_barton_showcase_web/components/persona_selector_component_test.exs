defmodule CadenBartonShowcaseWeb.PersonaSelectorComponentTest do
  ...
  test "renders labels and links to the correct sections" do
    ...
    developer_nodes = Floki.find(doc, "a[href='#case-study'][phx-hook='ScrollToSectionLink']")

    curious_nodes = Floki.find(doc, "a[href='#section-builds'][phx-hook='ScrollToSectionLink']")
    ...
    assert Floki.attribute(developer_nodes, "data-scroll-target") == ["#case-study"]
    assert Floki.text(developer_nodes) =~ "I’m a developer"

    assert length(curious_nodes) == 1
    assert Floki.attribute(curious_nodes, "data-scroll-target") == ["#section-builds"]
